...
- **Tagging benchmark** (📊 icon in the sidebar, or `/benchmark.html`): dry-run classifies every photo and video in a date range and compares against your actual Immich tags, so you can measure real recall and false-positive rates instead of guessing. Breaks results down by pet, photo/video, and YOLO detection confidence (strong/weak/fallback), with a summary table plus thumbnail grids of only the mismatches. CLIP threshold and YOLO detection confidence can be overridden per run to test different cutoffs, no effect on live tagging and nothing is saved, the overrides only apply to that one run. Results can be downloaded and re-imported later to skip re-running. Not part of the tagging pipeline, a diagnostic tool for tuning thresholds. Production `THRESHOLD`/`YOLO_CONF` are still configured via `docker-compose.yml` only.
