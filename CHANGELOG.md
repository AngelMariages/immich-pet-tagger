...
- README documents a "Bigger models" opt-in upgrade (`YOLO_MODEL_NAME=yolov8s.pt`, `CLIP_MODEL_NAME=ViT-L-14`, `CLIP_PRETRAINED=openai`) for users with GPU headroom, measured ~40% fewer missed tags and ~13% fewer false positives than the defaults on a real library. Not the shipped default, since it needs more RAM/VRAM and a larger model download.
